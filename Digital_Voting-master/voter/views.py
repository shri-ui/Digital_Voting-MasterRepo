...
def register(request):
    if (request.method == 'POST'):
        ...
        webcam_image_data = request.POST['webcam_image'].split(",")[1]  # Extract base64 image data
       
        webcam_image_bytes = base64.b64decode(webcam_image_data)
        v=Voters.objects.get(voterid_no=voter_id)
        Id=str(v.id)
        
        folder=str(BASE_DIR)+"/DatasetVideo/image1.jpg"
        with open(folder, "wb") as temp_image_file:
            temp_image_file.write(webcam_image_bytes)
        name=v.voterid_no
        ...
        recognizer.read(str(BASE_DIR)+"/TrainingImageLabel/Trainner.yml")
        faceDetect = cv2.CascadeClassifier(str(BASE_DIR)+"/haarcascade_frontalface_default.xml")
        
        img = cv2.imread(str(BASE_DIR)+v.voter_image.url)
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        faces = faceDetect.detectMultiScale(gray, 1.3, 5)
        flag=0
        for(x,y,w,h) in faces:
            ...
            if conf>50:
                if str(Id)==str(v.id):
                    tt="Detected"
                    cv2.putText(img, str(tt),(x,y+h), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,255,0),2)
                    cv2.waitKey(500)
                    flag=1
            else:
                Id='Unknown'
                tt=str(Id)
                cv2.putText(img, str(tt),(x,y+h), cv2.FONT_HERSHEY_SIMPLEX, 2, (0,0,255),2)
        cv2.imshow("Face",img)

        cv2.destroyAllWindows()
        
        if password1 == password2 and flag==1:
            print("a")
...
def velection(request):
    ...
    vdetail = Voters.objects.get(voterid_no=str(v_id))
        
    status = 'active'
    now = datetime.datetime.now()
    nowdate = now.strftime("%G-%m-%d")
    nowtime = now.strftime("%X")
    if Election.objects.filter(state=vdetail.state, status=status,start_date=nowdate,start_time__lt=nowtime,end_time__gt=nowtime):
        velection.e = Election.objects.get(state=vdetail.state, status=status,start_date=nowdate,start_time__lt=nowtime,end_time__gt=nowtime)
...
def subvoteemailotp(request):
    if request.method=="POST":
        ...
        if subvoteotp.otp==emailotp:
                # Assuming velection is the current election and vote is the current vote
            votecan = Votes.objects.get(election_id=velection.e.election_id, candidate_id=vote.candidateid)
            print(votecan.candidate_id)
